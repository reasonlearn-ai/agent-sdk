...
The budget cap covers [subagents](/docs/en/agent-sdk/subagents): their spend counts toward the total. {/_ min-version: 2.1.217 _/}Once spend reaches the cap, spawning another subagent fails with `Budget limit reached`, and Claude Code stops any background subagents still running. The cap-enforcement behaviors require Claude Code v2.1.217 or later.
